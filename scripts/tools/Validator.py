...
class Validator:
    ...
    def validate_advancement(
        cls, advancement: Advancement | InvalidAdvancement
    ) -> list[AdvWarning]:
        if isinstance(advancement, InvalidAdvancement):
            return [
                AdvWarning(
                    AdvWarningType.INVALID_ADVANCEMENT_IN_RELEASE,
                    f"Reason: {advancement.reason.reason}",
                )
            ]
    ...
    def _validate_reward_function(adv: Advancement) -> list[AdvWarning]:
        ...
        if not can_access_keypath(adv.json, ["rewards", "function"]):
        ...
        elif adv.reward_mcpath != (
            expect := f"{adv.datapack.reward_namespace}:{cut_namespace(adv.mc_path)}"
        ):
            warnings.append(
                AdvWarning(
                    AdvWarningType.REWARD_FUNCTION_DOESNT_MATCH,
            ...
            warnings.append(
                AdvWarning(
                    AdvWarningType.DONT_FILES_EXIST,
                    "Doesn't all files exist: " + ", ".join(files),
                )
            )
            ...
            warnings.append(
                AdvWarning(
                    AdvWarningType.EMPTY_FILES, "Some files empty: " + ", ".join(files)
                )
            )
    ...
    def _validate_parent(adv: Advancement) -> AdvWarning | None:
        if adv.filename == "root":
            return
        elif not AdvancementsManager.find(
            criteria={"mc_path": adv.parent},
            ...
            return AdvWarning(
                AdvWarningType.INVALID_PARENT, f"Invalid parent: {adv.parent}"
            )
        return
    ...
    def _find_last_advancement(adv: Advancement) -> Advancement:
        ...
        adv_map = {
            ...
            for adv in AdvancementsManager.filtered_iterator(
                datapack=DatapackList.available, skip_technical=False
            )
        }
    ...
    def _find_root_advancement(adv: Advancement) -> Advancement:
        ...
        """
        while adv.hidden:
            parent_advancements = AdvancementsManager.find(
                {"mc_path": adv.parent}, datapack=DatapackList.available, limit=1
            )
    ...
    def _validate_branch_existence(cls, adv: Advancement) -> AdvWarning | None:
        ...
        branch_parent_list = {
            tech_adv.parent
            for tech_adv in AdvancementsManager.filtered_iterator(
                datapack=DatapackList.available, skip_technical=False
            )
            if tech_adv.parent
        }
        ...
        if last_adv.mc_path not in branch_parent_list:
            ...
            if root_adv.mc_path not in branch_parent_list:
                return AdvWarning(
                    AdvWarningType.MISSING_BRANCH,
                    f"Doesn't have branch in {adv.tab} tab",
                )
    ...
    def validate_json_structure(
        adv_json: dict = None, datapack: Datapack = None
    ) -> List[AdvWarning]:
        """
        ...
        if frame and frame not in frame_list:
            warnings.append(
                AdvWarning(AdvWarningType.UNKNOWN_FRAME, f"Unknown frame: {frame}")
            )

        hidden = get_by_keypath(adv_json, ("display", "hidden")) or False

        if datapack.default_hidden_color and (
            hidden ^ (color == datapack.default_hidden_color)
        ):
            warnings.append(
                AdvWarning(
                    AdvWarningType.UNMATCHED_COLOR,
                    "The hidden color does not match the color",
                )
            )

        if not get_by_keypath(adv_json, ("display", "title", "translate")):
            warnings.append(
                AdvWarning(AdvWarningType.NO_TRANSLATE, "No 'translate' in title")
            )

        if not get_by_keypath(adv_json, ("display", "description", "translate")):
            warnings.append(
                AdvWarning(AdvWarningType.NO_TRANSLATE, "No 'translate' in description")
            )

        if not get_by_keypath(adv_json, ("parent",)) and not get_by_keypath(
            adv_json, ("background",)
        ):
            warnings.append(
                AdvWarning(
                    AdvWarningType.INVALID_PARENT,
                    "No 'parent' or 'background' (for root)",
                )
            )
...
class SpellingValidator:
    capitalized: Set[str] = set(
        json.loads(Path("resources/spelling/capitalized.json").read_text())
    )

    capitalized_ignore: Set[str] = set(
        json.loads(Path("resources/spelling/capitalized_ignore.json").read_text())
    )

    with_the: Set[str] = set(
        json.loads(Path("resources/spelling/with_the.json").read_text())
    )

    with_the_ignore: Set[str] = set(
    ...
    def validate_misspelling(cls, adv: Advancement) -> List[AdvWarning]:
        ...
        if (
            not cls._is_valid_title_case(adv.title)
            and adv.title not in cls.capitalized_ignore
        ):
            warnings.append(
                AdvWarning(
    ...
    @classmethod
    def _validate_capitalized(
        cls, string: str, place: Literal["title", "description"]
    ) -> List[AdvWarning]:
        ...
        for phrase in cls.capitalized:
            ...
            for match in matches:
                ...
                if original_phrase == phrase.title() or (
                    original_phrase.endswith("s")
                    and original_phrase[:-1] == phrase.title()
                ):
                    continue
                ...
                if (
                    phrase == "end"
                    and start_index > 0
                    and string[start_index - 4 : start_index].lower() != "the "
                ):
                    continue
    ...
    def _validate_the(
        ...
        for phrase in cls.with_the:
            ...
            for match in matches:
                ...
                if (
                    start_index > 0
                    and lowered_string[start_index - 4 : start_index] == "the "
                ):
                    continue
    ...
    def _validate_clean_edges(
        ...
        if string != stripped_string:
            if string.rstrip("\n") != string.rstrip("\n").lstrip():
                warnings.append(
                    AdvWarning(
                        AdvWarningType.MISSPELLING_ERROR,
                        f"{place} has wrong leading spaces",
                    )
                )
            ...
            if string.rstrip("\n") != string.rstrip("\n").rstrip():
                warnings.append(
                    AdvWarning(
                        AdvWarningType.MISSPELLING_ERROR,
                        f"{place} has wrong trailing spaces",
                    )
                )
        ...
        if stripped_string.endswith(".") and not stripped_string.endswith("..."):
            warnings.append(
                AdvWarning(
                    AdvWarningType.MISSPELLING_ERROR,
                    f"{place} has wrong dot at the end",
                )
            )
    ...
    def _validate_with_pattern(
        string: str, re_pattern: re.Pattern
    ) -> List[Tuple[int, str]]:
        if re_pattern.match(string):
            return []
        return [
            (i, char) for i, char in enumerate(string) if not re_pattern.search(char)
        ]
            ...
            if invalid_positions:
                message = (
                    ...
                    if len(invalid_positions) == 1
                        ...
                        f"'{char}' at position {index}"
                        for index, char in invalid_positions
                    )
                )
                warnings.append(
                    AdvWarning(
                        AdvWarningType.WRONG_SYMBOLS, f"Field '{field_name}': {message}"
                    )
                )
    ...
    def validate_title(cls, title: str, datapack: Datapack) -> list[AdvWarning]:
        ...
        if cls._validate_with_pattern(title, pattern):
            warnings.append(
                AdvWarning(
                    AdvWarningType.WRONG_SYMBOLS, f"Title has blacklisted symbols"
                )
            )
