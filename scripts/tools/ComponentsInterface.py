...
class ComponentsInterface:
    ...
    def __new__(
        cls, item_id, add_lore: bool = False, indent: int = 0
    ) -> Dict[str, Any]:
        """
        ...
        for component_class in cls.components_list:
            if hasattr(component_class, "always_set") and component_class.always_set:
                ...
                if get_bool(
                    f"Set {component_class.__name__} [y/n]:",
...
class Profile(BasicComponent):
    ...
    def input_component(self):
        ...
        search = None
        output(
            "https://minecraft-heads.com/wiki/heads/heads-categories",
            indent=self.indent,
        )
        while not search:
            get_value(
                "Copy the command to the clipboard and continue [any]:",
                indent=self.indent,
            )
        ...
        return {
            "profile": {"properties": [{"name": "textures", "value": search["value"]}]}
        }
...
class Trim(BasicComponent):
    ...
    def input_component(self):
        ...
        self.trim_pattern = get_value(
            "Trim:", possible_value=self.trim_list, indent=self.indent
        )
        self.trim_material = get_value(
            "Material:",
            possible_value=self.trim_material_colors.keys(),
            indent=self.indent,
        )
        comp = {
            "tooltip_display": {"hidden_components": []},
            "trim": {"material": self.trim_material, "pattern": self.trim_pattern},
        }
...
class BasicEnchantments(BasicComponent):
    ...
    def input_levels(self):
        while True:
            ...
            while True:
                enchantment = get_value(
                    "Enchantment:",
                    possible_value=Resources.Enchantments.list + [""],
                    indent=self.indent,
                )
                if enchantment == "":
                    break
                else:
                    levels[enchantment] = get_value(
                        "Level:", value_type=int, indent=self.indent
                    )
            output("Enchantments:\n" + json.dumps(levels, indent=1), indent=self.indent)
            if get_bool("Continue with these enchantments [y/n]:", indent=self.indent):
                self.show_in_tooltip = get_bool(
                    "Show in tooltip [y/n]:", indent=self.indent
                )
                if self.custom_lore and self.show_in_tooltip:
                    ...
                    "extra": [
                        {
                            ...
                            "color": "#FC5454"
                            if enchantment in Resources.Enchantments.curses
                            else "gray",
                            "italic": False,
                        }
                    ],
...
class StoredEnchantments(BasicEnchantments):
    ...
    def input_component(self):
        ...
        if not self.show_in_tooltip or self.custom_lore:
            comp["tooltip_display"]["hidden_components"].append(
...
class EnchantmentGlintOverride(BasicComponent):
    ...
    def input_component(self):
        return {
            "enchantment_glint_override": get_bool("Glint [y/n]:", indent=self.indent)
        }
        ...
        while True:
            try:
                self.color = get_value(
                    "Hex color:",
                    indent=self.indent,
                    function_to_check=Color.color_to_int,
                )
                break
            except ValueError:
                continue
...
class WritableBookContent(BookContent):
    ...
    def input_pages(self):
        comp = {"writable_book_content": {"pages": super().input_pages()}}
        return comp
...
class Bees(BasicComponent):
    ...
    def input_component(self):
        ...
        comp["bees"] = [bee_data] * get_value(
            "Bee count:", value_type=int, indent=self.indent
        )
        return comp
...
class OminusBottleAmplifier(BasicComponent):
    ...
    def input_component(self):
        comp = {
            "ominous_bottle_amplifier": get_value(
                "Ominus Level:", value_type=int, indent=self.indent
            )
        }
        return comp
...
class SuspiciousStewEffects(BasicComponent):
    ...
    def input_component(self):
        effect_id = get_value(
            "Effect:", possible_value=Resources.Effects.list, indent=self.indent
        )
    ...
    def input_component(self):
        ...
        comp = {
            "instrument": get_value(
                "Instrument:", possible_value=instrument_list, indent=self.indent
            )
        }
        return comp
...
class BaseColor(BasicComponent):
    ...
    def input_component(self):
        return {
            "base_color": get_value(
                "Color:", possible_value=Resources.DyeColors.list, indent=self.indent
            )
        }
...
class BannerPatterns(BasicComponent):
    ...
    def input_component(self):
        ...
        while True:
            pattern = get_value(
                "Pattern:", possible_value=self.pattern_list + [""], indent=self.indent
            )
            if not pattern:
                break
            patterns.append(
                {
                    "color": get_value(
                        "Color:",
                        possible_value=Resources.DyeColors.list,
                        indent=self.indent,
                    ),
                    "pattern": pattern,
                }
...
class FireworkExplosionBase(BasicComponent):
    ...
    def __get_list_color(self):
        color_list = []
        if get_bool(
            "Custom color or vanilla-color list [c/v]:",
            true_values="c",
            false_values="v",
            ...
            while True:
                color = get_value(
                    "Color:",
                    possible_value=set(self.dye_colors.keys())
                    | {
                        "",
                    },
                    indent=self.indent_expl_base + 3,
                ...
                output(
                    "Colors: " + ", ".join(color_list), indent=self.indent_expl_base + 3
                )
    ...
    def input_explosion(self):
        output(
            f"Possible shapes:\n" + ", ".join(self.possible_shapes),
            indent=self.indent_expl_base,
        )
        shape = get_value(
            "Shape:", possible_value=self.possible_shapes, indent=self.indent_expl_base
        )
    ...
    def input_component(self):
        ...
        comp = {
            ...
            "fireworks": {
                ...
                "flight_duration": get_value(
                    "Flight duration:", int, indent=self.indent
                ),
            },
        }
        if not self.show_in_tooltip or self.custom_lore:
...
class Container(ContainerBasic):
    ...
    def input_component(self):
        ...
        item_id = get_value(
            "Item id:", possible_value=Resources.ItemProperties.list, indent=self.indent
        )
            ...
            "container": [
                {
                    ...
                    "item": {
                        "id": item_id,
                        "count": (
                            min(item_stack_size, item_count - (slot * item_stack_size))
                        ),
...
class LodestoneTracker:
    ...
    def input_component(self):
        ...
        if get_bool("Add coordinates [y/n]:", indent=self.indent):
            x, y, z = (
                get_value("X:", int, indent=self.indent),
                get_value("Y:", int, indent=self.indent),
                get_value("Z:", int, indent=self.indent),
            )
            dimension = get_value(
                "Dimension:",
                possible_values=(
...
class BundleContents(BasicComponent):
    ...
    def input_component(self):
        ...
        while get_bool("Add Item [y/n]:", indent=self.indent):
            item_id = get_value(
                "Item id:",
                possible_value=Resources.ItemProperties.list,
                indent=self.indent + 3,
            )
            item_count = get_value(
                "Item count in container:", int, indent=self.indent + 3
            )
            components = {}
            ...
            container.append(
                {"id": item_id, "count": item_count, "components": components}
            )
        return {"bundle_contents": container}
...
class ChargedProjectiles(BasicComponent):
    ...
    def input_component(self):
        container = []
        output(
            "For a crossbow, you can add any number of projectiles", indent=self.indent
        )
        while get_bool("Add Projectile [y/n]:", indent=self.indent):
            ...
            item_count = get_value(
                "Count of this projectile:", int, indent=self.indent + 3
            )
            components = {}
            if get_bool(
                "Add components to this projectile [y/n]:", indent=self.indent + 3
            ):
                components = ComponentsInterface(item_id, indent=self.indent + 6)
            container.extend([{"id": item_id, "components": components}] * item_count)
        return {"charged_projectiles": container}
...
class PotionContents(BasicComponent):
    ...
    def input_component(self):
        ...
        custom_color = None
        output(
            "You can set any potion (one) and/or effects (any amount)",
            indent=self.indent,
        )
        if get_bool("Add custom potion [y/n]:", indent=self.indent):
            potion = get_value(
                "Potion:",
                possible_value=(self.potion_data.keys()),
                indent=self.indent + 3,
            )
            ...
            if possible:
                ...
                msg = (
                    " or ".join([prop.capitalize() for prop in possible] + ["Normal"])
                    + f" [{'/'.join(keys.keys())}/n]:"
                )
                potion = (
                    get_value_from_variants(msg, **keys, n="", indent=self.indent + 3)
                    + potion
                )

        if get_bool("Add custom color [y/n]:", indent=self.indent):
            custom_color = get_value(
                "Custom color:", value_type=Color.color_to_int, indent=self.indent + 3
            )
        ...
        while get_bool("Add effect [y/n]:", indent=self.indent):
            custom_effects.append(
                {
                    "id": get_value(
                        "Effect:", possible_value=self.effects, indent=self.indent + 3
                    ),
                    "amplifier": get_value("Amplifier:", int, indent=self.indent + 3),
                    "duration": get_value(
                        "Duration [seconds]:", int, indent=self.indent + 3
                    )
                    * 20,
                    "show_particles": get_bool(
                        "Show particle [y/n]:", indent=self.indent + 3
                    ),
                }
            )
        comp = {}
...
class PotDecoration(BasicComponent):
    ...
    def input_component(self):
        output(
            "Possible sherds: " + ", ".join(self.possible_sherds), indent=self.indent
        )
        sherds = []
        for _ in range(4):
            sherd = get_value(
                "Sherd:", possible_value=self.possible_sherds, indent=self.indent
            )
            if sherd != "brick":
    ...
    def input_component(self):
        ...
        while get_bool("Add modifiers [y/n]:", indent=self.indent):
            modifier = {
                "type": get_value(
                    "Modifier:",
                    possible_value=self.possible_modifiers,
                    indent=self.indent + 3,
                ),
                "operation": get_value(
                    "Operation:",
                ...
                "id": "",
                "slot": get_value(
                    "Slot:", possible_value=self.possible_slots, indent=self.indent + 3
                ),
            }
