...
class Datapack:
    ...
    def __init__(self, path_to_config_folder: Path):
        config = json.loads(
            (path_to_config_folder / "settings.json").read_text(encoding="utf-8")
        )
        ...
        if self._language_pack is not None:
            self._base_translation_path = (
                Path(config["base_translation_path"])
                if config.get("base_translation_path", None)
                else None
            )
            self._main_translation_path = (
                Path(config["main_translation_path"])
                if config.get("main_translation_path", None)
                else None
            )

        base_translation_header_path = (
            path_to_config_folder / "base_translation_header.txt"
        )
        self._base_translation_header = (
            base_translation_header_path.read_text()
            if base_translation_header_path.exists()
            else None
        )
        ...
        self._adv_parse_type_data = self.generate_adv_parse_type_data(
            config["adv_parse_type_data"]
        )
        ...
        self._release_name_pattern = (
            release_name_pattern_path.read_text(encoding=self._encoding)
            if release_name_pattern_path.exists()
            else None
        )

        install_mcfunc_pattern_path = path_to_config_folder / "install_mcfunc.pattern"
        self._install_mcfunc_pattern = (
            install_mcfunc_pattern_path.read_text(encoding=self._encoding)
            if install_mcfunc_pattern_path.exists()
            else None
        )
        ...
        self._default_hidden_color = config.get("default_hidden_data", {}).get(
            "color", "light_purple"
        )
        self._default_hidden_trophy_color = config.get("default_hidden_data", {}).get(
            "trophy_color", "#FFAEFF"
        )

        self._default_adv_namespace_path = (
            self._path / f"data/{self._default_adv_namespace}"
        )

        if self._legend_adv_mcpath:
            self._legend_adv_path = mc_path_to_path(
                self._path, "advancement", self._legend_adv_mcpath
            )
        else:
            self._legend_adv_path = None

        self._milestone_advs_path = {
            tab: mc_path_to_path(self._path, "advancement", adv)
            for tab, adv in self._milestone_adv_mcpaths.items()
        }

        self._adv_namespaces_paths = [
            self._path / f"data/{namespace}" for namespace in self._adv_namespaces
        ]
        self._default_advancements_path = (
            self._default_adv_namespace_path / "advancement"
        )

        self._advancement_paths = [
            namespace_path / "advancement"
            for namespace_path in self._adv_namespaces_paths
        ]

        self._function_path = (
            self._path / f"data/{self._default_adv_namespace}/function"
        )
        ...
        if self._install_mcpath:
            self._install_path = mc_path_to_path(
                self._path, "function", self._install_mcpath
            )
        else:
            self._install_path = None

        self._technical_paths = [
            self.default_advancements_path / tech_folder
            for tech_folder in config["technical_paths"]
        ]
        self._excluded_paths = [
            self.default_advancements_path / excluded_folder
            for excluded_folder in config["excluded_paths"]
        ]

        if config.get("use_default_msg", True):
            self._msg_patterns_path = (
                path_to_config_folder.parent / "default_msg"
            ).resolve()
        else:
            self._msg_patterns_path = path_to_config_folder / "msg"

        self._msg_patterns = {
            pattern.stem: pattern.read_text(encoding=self._encoding)
            for pattern in self._msg_patterns_path.iterdir()
            if pattern.suffix == ".pattern"
        }
        self._msg_milestone_names = json.loads(
            (self._msg_patterns_path / "milestone_names.json").read_text(
                encoding=self._encoding
            )
        )

        self._ignore_adv_gen_list: List[str] = json.loads(
            (path_to_config_folder / "ignore_adv_gen.json").read_text(encoding="utf-8")
        )

    def is_technical(self, path):
        return any(
            path.is_relative_to(tech_folder) for tech_folder in self.technical_paths
        )

    def is_excluded(self, path):
        return any(
            path.is_relative_to(excl_folder) for excl_folder in self.excluded_paths
        )

    def resolve_adv_type(
        self,
        ...
        for key, criteria in self.adv_parse_type_data.items():
            if (
                (filename is None or re.match(criteria["filename"], filename))
                and (tab is None or re.match(criteria["tab"], tab))
                and (color is None or re.match(criteria["color"], color))
                and (frame is None or re.match(criteria["frame"], frame))
                and (hidden is None or re.match(criteria["hidden"], str(hidden)))
            ):
                matches.append(key)
        if not matches:
            return None
...
class DatapackList:
    ...
    available: List[Datapack] = [
        Datapack(Path(f"config/{datapack}"))
        for datapack in datapack_list_json["available"]
    ]
    work_with: List[Datapack] = [
        Datapack(Path(f"config/{datapack}"))
        for datapack in datapack_list_json["work_with"]
    ]
