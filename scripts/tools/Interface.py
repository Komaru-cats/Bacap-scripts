...
literal_color_list = Literal[
    "black", "red", "green", "yellow", "blue", "purple", "cyan", "white"
]


def get_formatting(
    color: str = "white", bold: bool = False, italic: bool = False
) -> str:
    """
    Format string using ANSI.
    """
    font_dict = {"bold": "1", "italic": "3"}
    color_dict = {
        "black": "30",
        "red": "31",
        "green": "32",
...
class Icon:
    ...
    def __init__(
        self,
        icon: str,
        color: literal_color_list = "white",
        bold: bool = False,
        italic: bool = False,
    ):
...
class Interface:
    ...
    def __init__(
        self,
        default_color: literal_color_list = "white",
        default_bold: bool = False,
        default_italic: bool = False,
        output_icon: str = Icon.basic,
        input_icon: str = Icon.input,
        format_type_data: Dict[Type, Dict[str, str | bool]] = None,
    ):
        """
    ...
    def __get_text_properties(
        self, data, color=None, bold=None, italic=None
    ) -> Tuple[str, bool, bool]:
    ...
    def output(
        ...
        if isinstance(data, str) and len(data.splitlines()) > 1:
            ...
            lines_without_first = [
                (
                    " " * (len(icon) + indent + 1)
                    + self.text_wrapper(line, color, bold, italic)
                )
                for line in lines[1:]
            ]
            text = (
                self.text_wrapper(lines[0], color, bold, italic)
                + "\n"
    ...
    def get_value(
        self,
        msg: str,
        value_type: Type | Callable = None,
        possible_value: Iterable = None,
        indent: int = 0,
        icon: Icon = None,
        function_to_check: Callable = None,
        print_close: bool = True,
        close_n: int = 3,
        ...
        if len(msg.splitlines()) > 1:
            ...
            lines_without_first = [
                (" " * (len(icon) + indent + 1) + line) for line in lines[1:]
            ]
            msg = lines[0] + "\n" + "\n".join(lines_without_first)
        ...
        while True:
            ...
            if possible_value:
                if value not in possible_value:
                    if print_close:
                        variants = get_close_matches(
                            str(value), possible_value_text, close_n, cutoff=0.2
                        )
                        if variants:
                            self.output(
                                "Close possible: " + ", ".join(variants), indent=indent
                            )
                    continue
    ...
    def get_bool(
        ...
        indent: int = 0,
        icon: Icon = None,
        true_values: Iterable | str = None,
        false_values: Iterable | str = None,
        exit_on_empty: bool = False,
    ) -> bool:
        """
...
class MenuInterface:
    ...
    def menu(self, *args, **kwargs):
        ...
        names = self.func_separator.join(
            (func_data["name"] for func_data in self.funcs_menu.values())
        )
